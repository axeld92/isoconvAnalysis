...
%        
    ...
    obj_fun =@(parameters) sum((fsm(T{i},parameters)-da{i}).^2);% + sum((fsmi(T{i},parameters)-a{i}).^2));
%     valordeprueba = obj_fun(p0)
...
for i = 1:n %tasas de calentamiento
    for j = 1:3 %picos
        fitss{i}{j}(1) = dmax*fitss{i}{j}(1); %rescala dadt%
        fitss{i}{j}(3) = (T{i}(end) - T{i}(1))*fitss{i}{j}(3) + T{i}(1); %rescala temperatura
        fitss{i}{j}(4) = (T{i}(end) - T{i}(1))*fitss{i}{j}(4); %rescala ancho
...
grid
ylim([-0.002 0.012])
legend('Real','Ajustado','Pico 1','Pico 2','Pico 3')
...
for ii = 1:l
    ...
    else y(ii) = p(1)*exp(-4*log(2)*((T(ii)-p(3))/p(4))^2);
    end
end
 y=y';
%  
