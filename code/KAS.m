function EA = KAS(alphas,data_1,data_2,data_3,data_4)
...
x = [1/TT(1) ; 1/TT(2) ; 1/TT(3) ;1/TT(4)];
    y = [log(beta(1)/TT(1)^2); log(beta(2)/TT(2)^2) ; log(beta(3)/TT(3)^2) ; log(beta(4)/TT(4)^2)];
    [p,S]=polyfit(x,y,1);
    ci = polyparci(p,S);
    y1 = polyval(p,x);
    plot(x,y,'o')
