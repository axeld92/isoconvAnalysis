function EA = Starink3(alphas,data_1,data_2,data_3)
...
TT = [T_1(aa) T_2(aa) T_3(aa)];
%betai = [dadT1(TT(1)) dadT2(TT(2)) dadT3(TT(3)) dadT4(TT(4))];

x = [1/TT(1) ; 1/TT(2) ; 1/TT(3) ];
    y = [log(beta(1)/TT(1)^(1.92)); log(beta(2)/TT(2)^(1.92)) ; log(beta(3)/TT(3)^(1.92))];
    [p,S]=polyfit(x,y,1);
    ci = polyparci(p,S);
    y1 = polyval(p,x);
    ys(:,i) = y;
    ...
    CI(i) = abs(-ci(2,1)*8.314/1000/1.0008-Ee(i))/100;
end
EA = [Ee' CI'];
